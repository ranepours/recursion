...
function findFrom(board, word, y, x, seen){
	if(board[y][x] != word[0]) return false; //letter we're looking for cannot be found
    if(seen.has(y + '-' + x)) return false; //letter already used in current path
    if(word.length === 1) return true; //down to the last letter
